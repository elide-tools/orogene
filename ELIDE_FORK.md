...
- (2026-04-23) — `feat: port from async-std to tokio`
  ...
  - `async_std::fs::read_dir` stream API (which implements `futures::Stream`) replaced with `tokio::fs::read_dir` + `tokio_stream::wrappers::ReadDirStream` for pipeline-style usages (`.map().try_for_each_concurrent()`), and `rd.next_entry().await?` for while-loop usages. The stream-style combinators (`.map()`, `.try_for_each()`, `.try_for_each_concurrent()`) remain from `futures::TryStreamExt` since `ReadDirStream` implements `futures::Stream`.
  - `nassun/src/entries.rs` and `nassun/src/tarball.rs`: kept `futures::io::BufReader` (replacing `async_std::io::BufReader`) because `async_tar_wasm` and `async_compression::futures::bufread::GzipDecoder` operate in the `futures::AsyncRead` ecosystem. No `tokio_util::compat` bridge needed — `Tarball` implements `futures::AsyncRead` directly.
  - `async_tar_wasm 0.4.2-wasm.1` still pulls `async-std ≥1.6` transitively (it uses `async_std::path::Path` in its API). `Entry::path()` returns `Cow<'_, async_std::path::Path>`; bridged to `std::path::Path` via `p.as_os_str()` without naming `async_std` in first-party code.
  - `tokio::sync::Mutex::try_lock()` returns `Result<_, TryLockError>` (not `Option<_>` as in async-std); updated one call site in `resolver.rs` from `if let Some(g) = mutex.try_lock()` to `if let Ok(g) = mutex.try_lock()`.
  - `async-std` is no longer a direct dep of any first-party crate; it remains in the transitive graph via `nassun → async-tar-wasm → async-std`. All other async-std ecosystem crates (`async-io`, `async-channel`, `async-lock`, `event-listener`, `futures-lite`, `fastrand` old versions, etc.) survive only through that single `async-tar-wasm` chain and no longer appear as version-split duplicates.
